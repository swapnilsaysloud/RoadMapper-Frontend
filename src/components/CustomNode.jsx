...
const CustomNode = ({ data }) => {
    ...
    let firstLine = data.content ? data.content.split('\n')[0] : '';
  ...
  return (
    <div className={`p-4 rounded-lg shadow-md ${data.isCompulsory ? 'bg-blue-100 border-2 border-blue-500' : 'bg-gray-100 border border-gray-300'}`}>
      ...
      {data.isCompulsory && (
        <div className="mt-2 text-xs font-semibold text-blue-600">Learning</div>
      )}
      {!data.isCompulsory && (
        <div className="mt-2 text-xs font-semibold text-blue-600">Assignment</div>
      )}
